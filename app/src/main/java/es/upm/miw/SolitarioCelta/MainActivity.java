...
public class MainActivity extends AppCompatActivity {
    ...
    private final String CLAVE_PUNTUACION = "PUNTUACION_ACTUAL";
    private static final int RECUPERAR_REQUEST_CODE = 0;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        puntuacion = new Puntuacion (1, "Jugador", 0, formattedDateTime );
        mostrarTablero();
    }
    ...
    public void posicionPulsada(View v) {
        ...
        if (mJuego.juegoTerminado()) {
            new SalvarPuntuacionDialogFragment().show(getFragmentManager(), "SALVAR PUNTUACION DIALOG");
        }
    }

    public JuegoCelta getMJuego() {
        return this.mJuego;
    }
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        outState.putInt(CLAVE_PUNTUACION, puntuacion.getNumPiezas());
        super.onSaveInstanceState(outState);
    }

    public void onRestoreInstanceState(Bundle savedInstanceState) {
        super.onRestoreInstanceState(savedInstanceState);
        int numPiezas = savedInstanceState.getInt(CLAVE_PUNTUACION);
        String grid = savedInstanceState.getString(CLAVE_TABLERO);
        mJuego.deserializaTablero(grid);
        puntuacion.setNumPiezas(numPiezas);
        mostrarTablero();
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.menuReiniciar:
                mJuego.reiniciarPartida();;
                mostrarTablero();
                return true;

            case R.id.menuGuardar:

                new SalvarPartidaDialogFragment().show(getFragmentManager(), "SALVAR PARTIDA DIALOGO");
                mostrarTablero();
                return true;

            case R.id.menuRecuperar:
                this.mJuego.cargarPartida(this);
                return true;

            case R.id.menuMejoresResultados:
                this.mJuego.mejoresResultados(this);
                return true;
